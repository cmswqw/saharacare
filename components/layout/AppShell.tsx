...
export function AppShell({ children, mode = "patient", unreadNotifications = 0 }: { children: React.ReactNode; mode?: "patient" | "caregiver" | "doctor" | "admin"; unreadNotifications?: number }) {
  ...
  return <div className={patient ? "patient-ui" : ""}>{sidebar}<div className="lg:ps-72"><Header mode={mode} unreadCount={unreadNotifications} /><main className={patient ? "safe-bottom mx-auto max-w-7xl px-4 py-6 md:px-8 md:py-10" : "mx-auto max-w-7xl px-4 py-7 md:px-8 md:py-10"}>{children}</main></div>{patient ? <MobileBottomNavigation /> : null}</div>;
}
